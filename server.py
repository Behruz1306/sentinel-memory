...
def product_guide():
    ...
    return {
        ...
        "threat_memory": threat_memory.stats(),
        "nav_tour": [
            {"icon": "📊", "title": "Dashboard", "view": "dashboard",
             "where": "First item in the left menu",
             "purpose": "Your home screen: evaluation counts, recent sessions, and quick links to every test."},
            {"icon": "🧪", "title": "Playground", "view": "playground",
             "where": "Left menu → Playground",
             "purpose": "Freely adjust trust signals (identity, verification, origin, deepfake) and run the full pipeline in one click."},
            {"icon": "📖", "title": "How It Works", "view": "properties",
             "where": "Left menu → How It Works",
             "purpose": "Plain-language explanation of every Sentinel property and the permission matrix."},
            {"icon": "🔒", "title": "Evaluate Request", "view": "evaluate",
             "where": "Left menu → Evaluate Request",
             "purpose": "Multi-turn chat: type what a caller would say and watch ALLOW / REDACT / BLOCK verdicts with trust breakdown."},
            {"icon": "⚖️", "title": "Naive vs Sentinel", "view": "compare",
             "where": "Left menu → Naive vs Sentinel",
             "purpose": "Side-by-side proof: vanilla RAG leaks sensitive docs; Sentinel gates by session trust."},
            {"icon": "📁", "title": "Knowledge Base", "view": "knowledge",
             "where": "Left menu → Knowledge Base",
             "purpose": "Browse company documents with sensitivity tags (PUBLIC, INTERNAL, CONFIDENTIAL, FINANCIAL)."},
            {"icon": "📋", "title": "My Audits", "view": "audits",
             "where": "Left menu → My Audits",
             "purpose": "History of your saved evaluations with PDF and JSON audit exports."},
        ...
        "glossary": {
            ...
            "BLOCK": "Blocked — trust is too low. Sensitive content stays hidden from the agent.",
            "REDACT": "Partially hidden — personal data was masked before the agent could see the document.",
            "trust_score": "Session trust (0–100): how much Sentinel believes this caller. Higher = more access.",
            "naive_rag": "Naive RAG: a normal search that returns documents by text similarity only, with no security check.",
            "claimed_only": "Identity claimed in words only — not cryptographically verified.",
            "cryptographic": "Verified through SSO or a signed token — strong identity proof.",
            "spoofed": "Connection origin looks forged or untrusted.",
            "corporate_sso": "Caller authenticated through company single sign-on.",
            "voice_anomaly": "Synthetic or deepfake voice signal (0–100%). Higher = more likely fake audio.",
            "se_risk": "Social engineering risk: urgency, authority pressure, secrecy requests in the transcript.",
            "PUBLIC": "Open information — low trust required.",
            "INTERNAL": "Staff-only documents.",
            "CONFIDENTIAL": "Sensitive business data — high trust required.",
            "FINANCIAL": "Payroll, banking, wire details — highest trust required.",
        },
    }
